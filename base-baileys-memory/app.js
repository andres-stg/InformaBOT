...
// Cargar el Menú y las redes desde un archivo.txt
const menuPath = path.join(mensajesDir, "Republica.txt");
const menu = fs.readFileSync(menuPath, "utf8");

const redesPath = path.join(mensajesDir, "Redes.txt");
const redes = fs.readFileSync(redesPath, "utf8");

const boletinesPath = path.join(mensajesDir, "Boletines.txt")
const boletines = fs.readFileSync(boletinesPath, "utf8")

// Flujos hijos
const flowCursoFinal = addKeyword(EVENTS.ACTION)
    .addAnswer(readMessage('Testfinal.txt'), { capture: false }, async ({ }) => {
        return gotoFlow(flowMenu);
    });

const flowCurso7 = addKeyword(EVENTS.ACTION)
    .addAnswer(readMessage('Test7.txt'), { capture: true }, async (ctx, { flowDynamic, gotoFlow }) => {
...
function isValidResponse(response) {
    const validResponses = ['sí', 'Sí', 'SI', 'SÍ', 'si', 'Si', 'sÍ', 'sI', 'no', 'NO', 'No', 'nO', '1', '2', 'uno', 'Uno', 'UNO', 'dos', 'Dos', 'DOS', 'dOs', 'uNo', 'Vamos a darle', 'vamos a darle', 'Vamos a Darle', 'VAMOS A DARLE', 'Mejor después', 'Mejor despues', 'Mejor Despues', 'Mejor Despues', 'mejor despues','mejor después'];
    return validResponses.includes(response);
}

function isYesResponse(response) {
    const yesResponses = ['sí', 'Sí', 'SI', 'SÍ', 'si', 'sÍ', 'sI', 'Si', '1', 'uno', 'Uno', 'UNO', 'uNo', 'Vamos a darle', 'vamos a darle', 'Vamos a Darle', 'VAMOS A DARLE'];
    return yesResponses.includes(response);
}

function isNoResponse(response) {
    const noResponses = ['no', 'NO', 'No', 'nO', '2', 'dos', 'Dos', 'DOS', 'dOs'];
    return noResponses.includes(response);
}

// Flujos principales
const flowInvestiga = addKeyword(EVENTS.ACTION)
    .addAnswer('Puedes enviarme texto, imágenes, videos, documentos o notas de voz y lo guardaré en la base de datos para que nuestro equipo revise esa informacion y te contacte con la respuesta 👨🏻‍💻🔎👩🏽‍💻', { capture: true }, async (ctx, { db }) => {
        switch (ctx.type) {
            case 'imageMessage':
            case 'videoMessage':
...
const flowBoletin = addKeyword(EVENTS.ACTION)
    .addAnswer(boletines, { capture: false }, async (ctx, { gotoFlow }) => {
        return gotoFlow(flowMenu);
    });

const flowRedes = addKeyword(EVENTS.ACTION)
    .addAnswer(redes, { capture: false }, async (ctx, { gotoFlow }) => {
        return gotoFlow(flowMenu);
    });
...
const flowInicio = addKeyword(['datavenger', 'Datavenger', 'DATAVENGER', 'DatAvenger, DataAvenger, dataavenger, Dataavenger, dataAvenger'])
    ...
    .addAnswer('➡También podrás *sugerirme temas que quieres que investigue* 🤔', { delay: 2000 })
    .addAnswer('➡ Y además, tendrás la oportunidad de hacer el *Curso Express* para combatir la desinformación y convertirte en un *#HeroeXLaInformación* 🦸🏻‍♂🦸🏽‍♀', { delay: 3000 })
    .addAnswer('🤖 ¿En qué podemos ayudarte? Escribe *República* para ver las opciones', { delay: 2000, capture: true }, async (ctx, { gotoFlow }) => {
